...
class Landmark:
    ...
    ``material``, ``enterable`` and ``radius`` are what perception reports, and
    they are the difference between a place a being can ask about and a name
    floating over nothing. A world pack declares them; the simulation only
    carries them, exactly as it carries ``Entity.avatar_pack``.
    """
    ...
    #: How far the place extends from ``pos``. Walking to it arrives at the
    #: edge rather than the centre, which is what stops a being standing in
    #: the middle of a pond it only meant to reach.
    radius: float = 0.0
...
class World:
    ...
    #:
    #: Generated by :func:`andropia.worlds.describe.setting` from the world
    #: pack, never written by hand — the pack is also what the renderer draws
    #: from, so the two cannot disagree.
    setting: str = ""

    #: Which world pack this place came from, so the renderer can fetch the
    #: same declaration the description was generated from. An id, like
    #: ``Entity.avatar_pack``.
    world_pack: str = ""
